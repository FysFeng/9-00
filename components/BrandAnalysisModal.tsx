...
const TYPE_COLORS: Record<string, string> = {
    ...
    [NewsType.MARKET_SALES]: '#10b981',  // Emerald
...
const BrandAnalysisModal: React.FC<BrandAnalysisModalProps> = ({ isOpen, onClose, brand, allNews }) => {
    ...
    const milestones = useMemo(() => {
        ...
        return brandNews.filter(n =>
            n.type === NewsType.LAUNCH_PHYSICAL ||
            n.type === NewsType.POLICY ||
            n.type === NewsType.MARKET_SALES
                        ...
                        <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
                            {(['MTD', 'QTD', 'YTD', 'ALL'] as TimePeriod[]).map(t => (
                                <button
                                    key={t}
                                    onClick={() => setPeriod(t)}
                                    className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${period === t
                            ...
                            <div className="relative border-l-2 border-slate-100 ml-4 space-y-8 py-2">
                                {milestones.map((item) => (
                                    <div key={item.id} className="relative pl-8 group">
                                        {/* Dot */}
                                        <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-white shadow-sm z-10 transition-transform group-hover:scale-125 ${item.type === NewsType.LAUNCH_PHYSICAL ? 'bg-blue-500' :
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 mb-1">
                                            ...
                                            <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${item.type === NewsType.LAUNCH_PHYSICAL ? 'bg-blue-50 text-blue-600' :
