...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const NavItem = ({ view, icon, label }: { view: 'dashboard' | 'feed' | 'workbench', icon: React.ReactNode, label: string }) => (
    <button
      ...
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group mb-1 ${currentView === view
    ...
    <aside className="fixed left-0 top-0 h-full w-64 bg-white text-slate-800 flex flex-col z-50 border-r border-slate-200 shadow-sm">
            ...
            {availableBrands.map(brand => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs rounded-full border transition-all flex items-center gap-1 ${isEditingBrands
      ...
      <div className="p-4 border-t border-slate-200 bg-slate-50">
        <button
          ...
          className={`w-full py-3 rounded-lg border text-xs font-bold transition-all flex items-center justify-center gap-2 group ${currentView === 'workbench'
